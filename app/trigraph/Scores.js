...
export default function Scores() {
  ...
  if (!scores.length) return (
    <View style={styles.container}>
      <Text style={styles.noScores}>No Trigraph scores found!</Text>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Your Trigraph Scores</Text>
      <FlatList
        data={scores}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.text}>Sound: {item.type}</Text>
